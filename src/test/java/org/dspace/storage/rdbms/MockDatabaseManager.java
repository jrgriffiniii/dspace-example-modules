...
/**
 * Mocks a DatabaseManager to add some custom logic / queries to support the H2 in-memory database
 * for Unit Testing. By default DSpace does not fully support H2, so this MockDatabaseManager
 ...
 * @author tdonohue
 */
public final class MockDatabaseManager extends MockUp<DatabaseManager> {
  // Set our logger to specify the Mock class, so we know which logs are from the "real" vs "mock"
  // class
  private static final Logger log = Logger.getLogger(MockDatabaseManager.class);

  /**
   * Override/Mock the default "setConstraintDeferred()" method in order to add some custom
  ...
  /**
   * Override/Mock the default "setConstraintImmediate()" method in order to add some custom
   * H2-specific code (look for the comments with "H2" in them).
   *
  ...
  /**
   * Override/Mock the default "process()" method in order to add some custom H2-specific code (look
   * for the comments with "H2" in them below).
   *
   * <p>Convert the current row in a ResultSet into a TableRow object.
   *
   ...
   */
  @Mock
  static TableRow process(
      Invocation inv, ResultSet results, String table, List<String> pColumnNames)
      throws SQLException {
    // What type of database is this?
    String databaseType = DatabaseManager.getDbKeyword();
    ...
    if (databaseType != null && !databaseType.equals(DatabaseManager.DBMS_H2)) {
      ...
      // Now that we've prepped the TableRow, reset the flags so that we can detect which columns
      // have changed
      row.resetChanged();
      return row;
    }
